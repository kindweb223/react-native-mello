...
extension TextViewController: ShareNavigationable {
  ...
  func navigationDidTapRightButton() {
    ...
    if let attachementPath = attachementPath {
      delegate?.textViewController(self, didTapCreateCard: text ?? "", inFlow: selectedFlow(), withAttachement: attachementPath)
    } else {
      delegate?.textViewController(self, didTapCreateCard: text ?? "", inFlow: selectedFlow())
    }
  }
}
