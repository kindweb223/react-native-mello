...
class ImagePickerViewController: UIViewController {
  ...
  private var selectedCellModelsByOrder: [(asset: AssetLoader, selected: Bool, size: CGSize)] = []
...
extension ImagePickerViewController: UICollectionViewDelegate {
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    if cellModels[indexPath.item].selected {
      selectedCellModelsByOrder.append(cellModels[indexPath.item])
    } else if let index = selectedCellModelsByOrder.firstIndex(where: { $0.asset.url == cellModels[indexPath.item].asset.url }) {
      selectedCellModelsByOrder.remove(at: index)
    }
...
extension ImagePickerViewController: ShareNavigationable {    
  ...
  func navigationDidTapRightButton() {
    delegate?.imagePickerViewController(self, didSelectAssets: selectedCellModelsByOrder.map({ $0.asset }), fromParsedURL: parsedURL)
  }
}
